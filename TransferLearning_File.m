function [classes, predictedLabels_Test, score_Test, TestLabels, accuracy_Test, mynetOSAug, info] =... 
    TransferLearning_File(path, lgraph, patchSize, chnr, epochs, MiniBatchSize, InitialLearnRate,...
...
% Choosing the layers to be changed
names = {lgraph.Layers(end-2).Name, lgraph.Layers(end-1).Name, lgraph.Layers(end).Name};
lgraph = removeLayers(lgraph, names);
lastLayers = [
    fullyConnectedLayer(numClasses,'Name','fc','BiasLearnRateFactor',10)
...
% Connceting the new layers to the existing network
nameJoin = {lgraph.Layers(end-3).Name, lgraph.Layers(end-2).Name};
lgraph = connectLayers(lgraph,nameJoin{:});
